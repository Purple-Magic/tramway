...
RSpec.describe Kaminari::Helpers::Tag do
  describe '#page_url_for' do
    ...
    it 'builds path using custom_path_method with passed arguments' do
      ...
      tag.instance_variable_set(:@options, { custom_path_method: :post_path, custom_path_arguments: [post.id] })
      tag.instance_variable_set(:@params, params)

      expect(tag.page_url_for(2)).to eq(Tramway::Engine.routes.url_helpers.post_path(post.id, page: 2))
    end
