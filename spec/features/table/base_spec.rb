...
feature 'Table Base Spec', :js, type: :feature do
  ...
  scenario 'check table' do
    expect(page).to have_selector(".div-table.w-full.text-left.rtl\\:text-right.text-gray-500.dark\\:text-gray-400.mt-4")
  end
  ...
  scenario 'check cells' do
    expect(page).to have_selector('.div-table-cell.px-6.py-4.font-medium.text-gray-900.whitespace-nowrap.dark\\:text-white.text-xs.sm\\:text-base', count: User.count)
  end
end
